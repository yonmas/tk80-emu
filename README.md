...
実機に触れたことがない方向けに、操作説明カードの **ENG/JP** の右に **チュートリアル** ボタンを用意しています。押すと同じカードの位置に3ステップの目次が表示され、各ステップへ進めます。チュートリアルの開閉やステップの行き来はエミュレーターの状態に影響しません。エミュレーターへの入力が必要な内容（アドレスやバイト列）はコピーボタンでコピペできます。
...
- `src/tutorial.ts` — エミュレーターの下に表示する3ステップ・チュートリアル（目次 → 各ステップ、コピー機能）
